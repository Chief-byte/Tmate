...
{
	// load SP so
	if      (NULL != (m_hDll = dlopen("/usr/lib/liblfs.so", RTLD_LAZY)))		printf("LoadXFS(/usr/lib/liblfs.so) success\n");
	else if (NULL != (m_hDll = dlopen("/data/sp/liblfs.so", RTLD_LAZY)))		printf("LoadXFS(/data/sp/liblfs.so) success\n");
	else
	{
		printf("LoadXFS(/usr/lib/liblfs.so, /data/sp/liblfs.so) failed : %s\n", dlerror());
		return false;
	}
...
HRESULT CDC_30::OpenDC(LPCTSTR strServiceName, long dwVersionsRequired)
{
	...
	// 把_strDCName的后缀"_30"、"_31"、"_40"之类的去掉
	// 因为DBUS中注册的路径是不含后缀的，为了后续能从DBUS中获取到结果消息，需要去掉后缀
	...
	repalce_all_ditinct(strDCNameNoSuffix, string("_32"), string(""));
	repalce_all_ditinct(strDCNameNoSuffix, string("_40"), string(""));
	repalce_all_ditinct(strDCNameNoSuffix, string("_41"), string(""));
	repalce_all_ditinct(strDCNameNoSuffix, string("_42"), string(""));
	LPCTSTR strDCName = strDCNameNoSuffix.c_str();
	printf("<DC> DCName without suffix : %s\n", strDCName);

	m_strDBusPath = string("/lfs/") + strDCName + "/listener";
	printf("<DC> DCName : %s, DBusPath : %s)\n", m_strDCName.c_str(), m_strDBusPath.c_str());
	...
	if (FALSE == m_DCObject.m_bWosaXFSRun)
	{
		...
		// 20180521 利多富 PC2050 机器180秒内未能完成 取款 模块打开操作，更改为600秒，重试
		...
		*/
		...
		{
			// 非正常打开的m_DCObject.m_hOpenResult的LFS_SUCCESS结果不被认为是成功
			hResult = m_DCObject.m_hOpenResult;
		}
	}
	else
...
void * CDC_30::OpenThread(LPVOID lpParam)
{
	...
	Version.low_version = 0x01;
	Version.high_version = 0x01;
	hResult = LFSStartUp(dwVersionsRequired, &Version);
	printf("<DC> call LFSStartUp <<<\n");
	CString strTrace((Version.description == NULL ? 0 : strlen(Version.description))
					+ (Version.system_status == NULL ? 0 : strlen(Version.system_status))
					+ 2048, 0);
	sprintf((char *)strTrace.c_str(), "LFSStartUp():"
					...
					((Version.description==NULL)||(strcmp(Version.description,"")==0))?"NULL":Version.description,
					((Version.system_status==NULL)||(strcmp(Version.system_status,"")==0))?"NULL":Version.system_status);
    ...
    if (LFS_SUCCESS == hResult || LFS_ERR_ALREADY_STARTED == hResult)
    {
		...
		printf("LFSCreateAppHandle ret %d, pDC_30->m_DCObject.m_hApp = %d\n", hResult, pDC_30->m_DCObject.m_hApp);
		...
		*/
		SrvcVersion.version = 0x01;
		SrvcVersion.low_version = 0x01;
		SrvcVersion.high_version = 0x01;
		...
		*/
		...
		pDC_30->m_bIsOenCompleted = FALSE;  // 后续需要等待SP的模块打开完成事件
		...
		pDC_30->m_DCObject.m_hOpenResult = hResult =
			LFSAsyncOpen(lpszLogicalName,
				...
				LFS_INDEFINITE_WAIT,
				&(pDC_30->m_DCObject.m_hService),
				(char *)pDC_30->m_strDBusPath.c_str(),
				dwVersionsRequired,
				&SrvcVersion,
				&SPIVersion, 
				&requestid);
		...
		strTrace = CString((lpszLogicalName == NULL ? 0 : strlen(lpszLogicalName))
							...
							+ (Version.description == NULL ? 0 : strlen(Version.description))
							+ (Version.system_status == NULL ? 0 : strlen(Version.system_status))
							+ 2048, 0);
		...
		sprintf((char *)strTrace.c_str(), "LFSAsyncOpen():"
						...
						dwVersionsRequired,
						SrvcVersion.version,
						SrvcVersion.low_version,
						SrvcVersion.high_version,
						((Version.description==NULL)||(strcmp(Version.description,"")==0))?"NULL":Version.description,
						((Version.system_status==NULL)||(strcmp(Version.system_status,"")==0))?"NULL":Version.system_status);
		WriteTrace(pDC_30->m_strTraceComponent.c_str(), XFSApiOpen, strTrace.c_str());
		...
		if (LFS_SUCCESS == hResult)
		{
			...
			{
				// 20240821 原150秒，怡化南京银行金胜路支行记录启动时间156秒，更改为180秒
...
LPTSTR CDC_30::FormatExtra(LPTSTR lpszExtra)
{
	// 格式化扩展字段数据
	// 大部分情况下，扩展字段以\0结束
	// 部分情况下，扩展字段以连续两个\0结束，中间的单个\0只用于分割扩展字段中的多段数据，如果SP不是这种情况按照这样处理可能指针越界异常崩溃
	// 这两种情况不兼容：如果存在文件 /SSA/WSServer/extra2null.flag 时按照连续两个\0结束处理
	struct stat s_buf = { 0 };
	...
	if (0 == stat("/SSA/WSServer/extra2null.flag", &s_buf))
	{
		for (int nLoop = 0; ; nLoop++)
		{
			if ((NULL == lpszExtra[nLoop] && NULL == lpszExtra[nLoop + 1])
				|| lpszExtra[nLoop] > 128)
			{
				// 遇到连续两个NULL时结束，或遇非可见英文字符时结束(防止读到乱码)
				break;
			}
	...
	else
	{
		for (int nLoop = 0; ; nLoop++)
		{
			if (NULL == lpszExtra[nLoop]
				|| lpszExtra[nLoop] > 128)
			{
				// 遇到连续NULL时结束，或遇非可见英文字符时结束(防止读到乱码)
				break;
			}
			else
			{
