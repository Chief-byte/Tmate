...
public:
	...
	HRESULT INF_CIM_TELLER_INFO();  // 暂未实现
	HRESULT INF_CIM_CURRENCY_EXP();
	HRESULT INF_CIM_BANKNOTE_TYPES();
	HRESULT INF_CIM_CASH_IN_STATUS();
	HRESULT INF_CIM_GET_P6_INFO();  // 暂未实现
	HRESULT INF_CIM_GET_P6_SIGNATURE();  // 暂未实现
	...
	HRESULT CMD_CIM_SET_CASH_IN_LIMIT(long lTotalItemsLimit, LPCTSTR strCurrencyID, long lAmount);
	HRESULT CMD_CIM_CASH_UNIT_COUNT(LPCTSTR strCashUnitNumbers);
