...
CString CDCPinPad_30::HandleInfResult(LPLFSRESULT lpWfsRet)
{
	...
	if (LFS_INF_PIN_STATUS == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSPINSTATUS lpStatus = (LPLFSPINSTATUS)lpWfsRet->lpvdBuffer;
		if (NULL != lpStatus)
		{
			strWfsResult = CString((NULL == lpStatus->lpstrExtra ? 0 : strlen(lpStatus->lpstrExtra)) + 1024, 0);
	...
	else if (LFS_INF_PIN_CAPABILITIES == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSPINCAPS lpCaps = (LPLFSPINCAPS)lpWfsRet->lpvdBuffer;
		if (NULL != lpCaps)
		{
			strWfsResult = CString((NULL == lpCaps->lpstrExtra ? 0 : strlen(lpCaps->lpstrExtra)) + 2048, 0);
			sprintf((char *)strWfsResult.c_str(), "wfsresult.u.dwcommandcode=%d"
				"%s"
				...
				lpCaps->wPin_Formats,
				GetStringSplitChar(),
				lpCaps->wDerivation_Algorithms,
				GetStringSplitChar(),
				lpCaps->wPresentation_Algorithms,
				GetStringSplitChar(),
				lpCaps->wDisplay,
				GetStringSplitChar(),
				lpCaps->bID_Connect ? "true" : "false",
				GetStringSplitChar(),
				lpCaps->wID_Key,
				GetStringSplitChar(),
				lpCaps->wValidation_Algorithms,
				GetStringSplitChar(),
				lpCaps->wKey_Check_Modes,
				GetStringSplitChar(),
				(NULL == lpCaps->lpstrExtra ? "NULL" : FormatExtra(lpCaps->lpstrExtra)));
		}
	}
	else if (LFS_INF_PIN_FUNCKEY_DETAIL == lpWfsRet->u.dwCommand_Code)
	{
		...
		if (NULL != lpFK)
		{
			...
			for (int nLoop = 0; nLoop < lpFK->usNumber_FDKs; nLoop++)
			{
				...
				sprintf((char *)strFunctionPad.c_str(), "%s"
					...
					GetStringSplitChar(),
					nLoop, lpFK->lppFDKs[nLoop]->ulFDK);

				strWfsResult = string(strWfsResult.c_str()) + strFunctionPad;
			}
		}
	}
	else if (LFS_INF_PIN_KEY_DETAIL == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSPINKEYDETAIL * lppKDetails = (LPLFSPINKEYDETAIL *)lpWfsRet->lpvdBuffer;
		if (NULL != lppKDetails)
		{
			for (int nLoop = 0; NULL != lppKDetails[nLoop]; nLoop++)
			{
				CString strKDetails((NULL == lppKDetails[nLoop]->lpstrKey_Name ? 0 : strlen(lppKDetails[nLoop]->lpstrKey_Name)) + 2048, 0);
				sprintf((char *)strKDetails.c_str(), "%s"
					...
					GetStringSplitChar(),
					nLoop, (NULL == lppKDetails[nLoop]->lpstrKey_Name ? "NULL" : lppKDetails[nLoop]->lpstrKey_Name),
					GetStringSplitChar(),
					nLoop, (TRUE == lppKDetails[nLoop]->bLoaded ? "true" : "false"),
					GetStringSplitChar(),
					nLoop, lppKDetails[nLoop]->wUse);
...
HRESULT CDCPinPad_30::CMD_PIN_CRYPT(LPCTSTR strBytesHex, LPCTSTR strKeyName, long wMode, long wAlgorithm, short chPadChar) 
{
	...
	lpCrypt->lpstrKey = new char[strlen(strKeyName) + 1];
	memset(lpCrypt->lpstrKey, 0, strlen(strKeyName) + 1);

	lpCrypt->lpxCrypt_Data = new LFSXDATA();
	memset(lpCrypt->lpxCrypt_Data, 0, sizeof(LFSXDATA));

	lpCrypt->lpxCrypt_Data->lpbData = new BYTE[strlen(strBytesHex) / 2];
	memset(lpCrypt->lpxCrypt_Data->lpbData, 0, strlen(strBytesHex) / 2);
	...
	lpCrypt->wMode = (WORD)wMode;
	strcpy((char*)lpCrypt->lpstrKey, strKeyName);
	lpCrypt->lpxKey_EncKey = NULL;
	// lpCrypt->algorithm = (WORD)wAlgorithm;
	lpCrypt->wAlgorithm = LFS_PIN_CRYPTSM4;  // �̶�ʹ�ù����㷨SM4
	lpCrypt->lpstrStart_ValueKey = NULL;
	lpCrypt->lpxStart_Value = NULL;
	lpCrypt->bPadding = (BYTE)chPadChar;
	lpCrypt->bCompression = 0x00;
	
	lpCrypt->lpxCrypt_Data->usLength = strlen(strBytesHex) / 2;
	memcpy((char *)lpCrypt->lpxCrypt_Data->lpbData, pszValue, lpCrypt->lpxCrypt_Data->usLength);
	...
	delete[] lpCrypt->lpxCrypt_Data->lpbData;
	// WFMFreeBuffer(lpCrypt->crypt_data);
	delete lpCrypt->lpxCrypt_Data;
	// WFMFreeBuffer(lpCrypt->key);
	delete[] lpCrypt->lpstrKey;
	// WFMFreeBuffer(lpCrypt);
	delete lpCrypt;
...
HRESULT CDCPinPad_30::CMD_PIN_GET_DATA(short nMaxLength, short nAutoEnd, ULONG active_keys, ULONG active_fdks, ULONG terminate_keys, ULONG terminate_fdks)
{
	...
	lpPinGetData->usMax_Len = nMaxLength;
	if (nAutoEnd == 0)
	{
		lpPinGetData->bAuto_End = FALSE;
	}
	else
	{
		lpPinGetData->bAuto_End = TRUE;
	}

	lpPinGetData->ulTerminate_Keys = terminate_keys;
	lpPinGetData->ulTerminate_FDKs = terminate_fdks;
	lpPinGetData->ulActive_Keys = active_keys | lpPinGetData->ulTerminate_Keys;
	lpPinGetData->ulActive_FDKs = active_fdks | lpPinGetData->ulTerminate_FDKs;
...
HRESULT CDCPinPad_30::CMD_PIN_GET_PIN(short nMinLength, short nMaxLength, short nAutoEnd, ULONG active_keys, ULONG active_fdks, ULONG terminate_keys, ULONG terminate_fdks) 
{
	...
	lpGetPin->usMin_Len = nMinLength;
	lpGetPin->usMax_Len = nMaxLength;

	if (0 == nAutoEnd)
	{
		lpGetPin->bAuto_End = FALSE;
	}
    else
	{
		lpGetPin->bAuto_End = TRUE;
	}
	
	lpGetPin->cEcho = '*';
	
	lpGetPin->ulTerminate_Keys = terminate_keys;
	lpGetPin->ulTerminate_FDKs = terminate_fdks;
	lpGetPin->ulActive_Keys = active_keys | lpGetPin->ulTerminate_Keys;
	lpGetPin->ulActive_FDKs = active_fdks | lpGetPin->ulTerminate_FDKs;
...
HRESULT CDCPinPad_30::CMD_PIN_GET_PINBLOCK(WORD format, LPCTSTR strCustomerData, short chPadChar, LPCTSTR strKeyName, LPCTSTR strXORData, LPCTSTR strSecondKeyName) 
{
	...
	if (strlen(_strCustomerData.c_str()) > 5)
	{
		_strCustomerData = _strCustomerData.substr(0, strlen(_strCustomerData.c_str()) - 5) + "****" + _strCustomerData.substr(strlen(_strCustomerData.c_str()) - 1, 1);
	}
	...
	if (NULL != strKeyName && strlen(strKeyName) > 0)
	{
		...
		lpPinBlock->lpstrKey = new char[strlen(strKeyName) + 3 + 1];
		memset(lpPinBlock->lpstrKey, 0, strlen(strKeyName) + 3 + 1);

		strcpy(lpPinBlock->lpstrKey, strKeyName);
	}
	else
	{
		lpPinBlock->lpstrKey = NULL;
	}
	...
	if (NULL != strCustomerData && strlen(strCustomerData) > 0)
	{
		...
		lpPinBlock->lpstrCustomer_Data = new char[strlen(strCustomerData) + 1];
		memset(lpPinBlock->lpstrCustomer_Data, 0, strlen(strCustomerData) + 1);

		strcpy(lpPinBlock->lpstrCustomer_Data, strCustomerData);
	}
	else
	{
		lpPinBlock->lpstrCustomer_Data = NULL;
	}
	
	if (NULL != strXORData && strlen(strXORData) > 0)
	{
		// WFMAllocateBuffer(strlen(strXORData) + 1, LFS_MEM_ZEROINIT, (void **)&lpPinBlock->xor_data);
		lpPinBlock->lpstrXOR_Data = new char[strlen(strXORData) + 1];
		memset(lpPinBlock->lpstrXOR_Data, 0, strlen(strXORData) + 1);

		strcpy(lpPinBlock->lpstrXOR_Data, strXORData);
	}
	else
	{
		lpPinBlock->lpstrXOR_Data = NULL;
	}
	...
	if (NULL != strSecondKeyName && strlen(strSecondKeyName) > 0)
	{
		...
		lpPinBlock->lpstrKey_EncKey = new char[strlen(strSecondKeyName) + 1];
		memset(lpPinBlock->lpstrKey_EncKey, 0, strlen(strSecondKeyName) + 1);

		strcpy(lpPinBlock->lpstrKey_EncKey, strSecondKeyName);
	}
	else
	{
		lpPinBlock->lpstrKey_EncKey = NULL;
	}
	
	lpPinBlock->bPadding = (BYTE)chPadChar;
	lpPinBlock->wFormat = format;
	// lpPinBlock->algorithm = LFS_PIN_CRYPTSM4; // �̶�ʹ�ù����㷨SM4��PISA��ȥ�����������
	...
	if (NULL != lpPinBlock->lpstrKey)
	{
		// WFMFreeBuffer(lpPinBlock->key);
		delete[] lpPinBlock->lpstrKey;
	}

	if (NULL != lpPinBlock->lpstrKey_EncKey)
	{
		// WFMFreeBuffer(lpPinBlock->key_enc_key);
		delete[] lpPinBlock->lpstrKey_EncKey;
	}

	if (NULL != lpPinBlock->lpstrXOR_Data)
	{
		// WFMFreeBuffer(lpPinBlock->xor_data);
		delete[] lpPinBlock->lpstrXOR_Data;
	}

	if (NULL != lpPinBlock->lpstrCustomer_Data)
	{
		// WFMFreeBuffer(lpPinBlock->customer_data);
		delete[] lpPinBlock->lpstrCustomer_Data;
	}
...
HRESULT CDCPinPad_30::CMD_PIN_IMPORT_KEY(LPCTSTR strKeyName, LPCTSTR strBytesHexKeyValue, LPCTSTR strEncKeyName, DWORD fwUse, LPCTSTR strBytesHexIdentification) 
{
	...
	if (NULL != strBytesHexIdentification && strlen(strBytesHexIdentification) > 0)
	{
		...
		lpImport->lpxIdent = new LFSXDATA();
		memset(lpImport->lpxIdent, 0, sizeof(LFSXDATA));

		// WFMAllocateBuffer(64, LFS_MEM_ZEROINIT, (void **)&lpImport->ident->data);
		lpImport->lpxIdent->lpbData = new BYTE[64];
		memset(lpImport->lpxIdent->lpbData, 0, 64);

		lpImport->lpxIdent->usLength = strlen(strBytesHexIdentification) / 2;
		memcpy((CHAR *)lpImport->lpxIdent->lpbData, achIdent, lpImport->lpxIdent->usLength);
	}
	else
	{
		lpImport->lpxIdent = NULL;
	}

	// WFMAllocateBuffer(sizeof(LFSXDATA), LFS_MEM_ZEROINIT, (void **)&lpImport->value);
	lpImport->lpxValue = new LFSXDATA();
	memset(lpImport->lpxValue, 0, sizeof(LFSXDATA));

	// WFMAllocateBuffer(64, LFS_MEM_ZEROINIT, (void **)&lpImport->value->data);
	lpImport->lpxValue->lpbData = new BYTE[64];
	memset(lpImport->lpxValue->lpbData, 0, 64);

	lpImport->lpxValue->usLength = strlen(strBytesHexKeyValue) / 2;
	memcpy((char *)lpImport->lpxValue->lpbData, achKeyValue, lpImport->lpxValue->usLength);

	// WFMAllocateBuffer(64, LFS_MEM_ZEROINIT, (void **)&lpImport->key);
	lpImport->lpstrKey = new char[64];
	memset(lpImport->lpstrKey, 0, 64);

	strcpy((char *)lpImport->lpstrKey, strKeyName);

	if (NULL != strEncKeyName && strlen(strEncKeyName) > 0)
	{
		// WFMAllocateBuffer(64, LFS_MEM_ZEROINIT, (void **)&lpImport->enc_key);
		lpImport->lpstrEnc_Key = new char[64];
		memset(lpImport->lpstrEnc_Key, 0, 64);

		strcpy(lpImport->lpstrEnc_Key, strEncKeyName);
	}
	else
	{
		lpImport->lpstrEnc_Key = NULL;
	}

	// lpImport->wUse = (WORD)(fwUse | LFS_PIN_USESM4);
	lpImport->wUse = (WORD)fwUse;  // PISAȥ���� LFS_PIN_USESM4 �Ķ���
	...
	if (NULL != strBytesHexIdentification && strlen(strBytesHexIdentification) > 0)
	{
		...
		delete[] lpImport->lpxIdent->lpbData;

		// WFMFreeBuffer(lpImport->ident);
		delete lpImport->lpxIdent;
	}

	// WFMFreeBuffer(lpImport->value->data);
	delete[] lpImport->lpxValue->lpbData;

	// WFMFreeBuffer(lpImport->value);
	delete lpImport->lpxValue;

	// WFMFreeBuffer(lpImport->key);
	delete[] lpImport->lpstrKey;

	if (NULL != strEncKeyName && strlen(strEncKeyName) > 0)
	{
		// WFMFreeBuffer(lpImport->enc_key);
		delete[] lpImport->lpstrEnc_Key;
	}
...
CString CDCPinPad_30::HandleCmdResult(LPLFSRESULT lpWfsRet)
{
	...
	HRESULT hResult = lpWfsRet->hResult;
	...
	DWORD dwCmdCode = lpWfsRet->u.dwCommand_Code;
	...
	switch (lpWfsRet->u.dwCommand_Code)
	{
		...
		{
			LPLFSXDATA lpxCryptData = (LPLFSXDATA)lpWfsRet->lpvdBuffer;
			if (NULL != lpxCryptData && lpxCryptData->usLength > 0)
			{
				char * achHexStr = new char[lpxCryptData->usLength * 2 + 2];
				memset(achHexStr, 0, (lpxCryptData->usLength * 2 + 2) * sizeof(char));
				m_Utils.HextoStr(	lpxCryptData->lpbData,
									lpxCryptData->usLength,
									achHexStr);

				strCmdResult = CString(strlen(achHexStr) + 256, 0);
		...
		{
			LPLFSPINENTRY lpEntry = (LPLFSPINENTRY)lpWfsRet->lpvdBuffer;
			if (NULL != lpEntry)
			{
		...
		case LFS_CMD_PIN_GET_DATA:
		{
			...
			LPLFSPINDATA lpPinData = (LPLFSPINDATA)lpWfsRet->lpvdBuffer;
			if (NULL != lpPinData)
			{
				strCmdResult = CString(256, 0);
				sprintf((char *)strCmdResult.c_str(), "%s"
									...
									lpPinData->usKeys,
									GetStringSplitChar(),
									lpPinData->wCompletion);

				for (int nLoop = 0; nLoop < lpPinData->usKeys; nLoop ++)
				{
					CString strTemp(256, 0);
					sprintf((char *)strTemp.c_str(), "%s"
									"wfsresult.lpbuffer.lppinkeys[%d].uldigit=%d",
									GetStringSplitChar(),
									nLoop, lpPinData->lpPin_Keys[nLoop]->ulDigit);
		...
		{
			LPLFSXDATA lpxPinBlock = (LPLFSXDATA)lpWfsRet->lpvdBuffer;
			if (NULL != lpxPinBlock)
			{
				char achPINBlock[64] = { 0 };

				m_Utils.HextoStr(lpxPinBlock->lpbData, lpxPinBlock->usLength, achPINBlock);
		...
		case LFS_CMD_PIN_ENC_IO:
		{
			LPLFSPINENCIO lpEncIoOut = (LPLFSPINENCIO)lpWfsRet->lpvdBuffer;
			if (NULL != lpEncIoOut)
			{
				strCmdResult = CString(256, 0);
				sprintf((char *)strCmdResult.c_str(), "%s"
					...
					GetStringSplitChar(),
					lpEncIoOut->wProtocol,
					GetStringSplitChar(),
					lpEncIoOut->ulDataLength);

				if (NULL != lpEncIoOut->lpvData)
				{
					LPPROTCHNIMPORTSM2PUBLICKEYOUT lpKeyOut = (LPPROTCHNIMPORTSM2PUBLICKEYOUT)lpEncIoOut->lpvData;
					if (NULL != lpKeyOut)
					{
						int usLength = 0;
						...
						{
							usLength = lpKeyOut->lpxKeyCheckValue->usLength;

							m_Utils.HextoStr(lpKeyOut->lpxKeyCheckValue->lpbData,
								usLength,
								achDataHexStr);
						}
		...
		case LFS_CMD_PIN_DERIVE_KEY:
		{
			LPLFSXDATA lpxCryptDataOut = (LPLFSXDATA)lpWfsRet->lpvdBuffer;
			if (NULL != lpxCryptDataOut)
			{
				char achDataHexStr[1024] = { 0 };
				m_Utils.HextoStr(lpxCryptDataOut->lpbData,
					lpxCryptDataOut->usLength,
					achDataHexStr);
...
CString CDCPinPad_30::HandleEXEEvtResult(LPLFSRESULT lpWfsRet)
{
	...
	switch (lpWfsRet->u.dwEvent_ID)
	{
		...
		{
			LPLFSPINKEY lpKey = (LPLFSPINKEY)lpWfsRet->lpvdBuffer;
			if (NULL != lpKey)
			{
...
HRESULT CDCPinPad_30::CMD_PIN_ENC_IO(long lCommand, LPCTSTR strKeyName, LPCTSTR strDataHex, long lUse, LPCTSTR strSigKey, long lSM2SignatureAlgorithm, LPCTSTR strSignatureHex)
{
	...
	InData.wCommand = (WORD)lCommand;  // WFS_CMD_ENC_IO_CHN_IMPORT_SM2_PUBLIC_KEY ���� WFS_CMD_ENC_IO_CHN_IMPORT_RSA_PUBLIC_KEY

	InData.lpsKey = (LPTSTR)(LPCTSTR)strKeyName;  // ��Կ����
	InData.dwUse = (WORD)lUse;  // ��Կ�÷���PROT_CHN_USESM2PUBLIC ���� WFS_PIN_USERSAPUBLIC
	...
	wfsxValue.lpbData = pszValue;
	wfsxValue.usLength = nLenBytes;
	...
	if (NULL != strSignatureHex && 0 != strlen(strSignatureHex))
	{
		...
		wfsxSignature.lpbData = pszSignatureValue;
		wfsxSignature.usLength = nLenSignature;

		InData.lpxSignature = &wfsxSignature;
	}

	pinEncIo.wProtocol = LFS_PIN_ENC_PROT_CHN;
	pinEncIo.ulDataLength = sizeof(InData);
	pinEncIo.lpvData = &InData;
...
HRESULT CDCPinPad_30::CMD_PIN_DERIVE_KEY(long wDerivationAlgorithm, LPCTSTR strKeyName, LPCTSTR strKeyGenKey, LPCTSTR strStartValueKey, LPCTSTR strBytesHexStartValue, long lPadding, LPCTSTR strBytesHexInputData, LPCTSTR strBytesHexIdentification)
{
	...
	pinderive.wDerivation_Algorithm = (WORD)wDerivationAlgorithm;  // WFS_PIN_KEYDES ���� WFS_PIN_KEYSM4
	...
	if (NULL != strBytesHexStartValue && 0 != strlen(strBytesHexStartValue))
	{
		...
		wfsxStartValue.lpbData = pszStartValue;
		wfsxStartValue.usLength = nLenStartValue;

		pinderive.lpxStart_Value = &wfsxStartValue;
	}

	pinderive.bPadding = (BYTE)lPadding;

	BYTE * pszInputData = NULL;
	pinderive.lpxInput_Data = NULL;
	...
	if (NULL != strBytesHexInputData && 0 != strlen(strBytesHexInputData))
	{
		...
		wfsxStartValue.lpbData = pszInputData;
		wfsxStartValue.usLength = nLenInputData;

		pinderive.lpxInput_Data = &wfsxInputData;
	}

	BYTE * pszIdent = NULL;
	pinderive.lpxIdent = NULL;
	...
	if (NULL != strBytesHexIdentification && 0 != strlen(strBytesHexIdentification))
	{
		...
		wfsxStartValue.lpbData = pszIdent;
		wfsxStartValue.usLength = nLenIdent;

		pinderive.lpxIdent = &wfsxIdent;
	}
...
HRESULT CDCPinPad_30::CMD_PIN_DERIVE_KEY(long wDerivationAlgorithm, LPCTSTR strKeyName, LPCTSTR strKeyGenKey, LPCTSTR strStartValueKey, LPCTSTR strBytesHexStartValue, long lPadding, LPCTSTR strBytesHexInputData, LPCTSTR strBytesHexIdentification)
{
	...
	LFSPINDERIVE pinderive = { 0 };
	pinderive.derivation_algorithm = (WORD)wDerivationAlgorithm;  // WFS_PIN_KEYDES ���� WFS_PIN_KEYSM4
