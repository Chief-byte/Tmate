...
HRESULT CDCCamera_31::CMD_CAM_TAKE_PICTURE(long lCamera, LPCTSTR lpszCamData, LPCTSTR lpszUNICODECamData, LPCTSTR lpszFilePath, long lScanSize)
{
	...
	TakePictEx.lpstrCam_Data = NULL;
	TakePictEx.lpstrUNICODE_Cam_Data = NULL;
	if (strlen(lpszCamData) > 0)
	{
		TakePictEx.lpstrCam_Data = (LPTSTR)lpszCamData;
	}
	else if (strlen(lpszUNICODECamData) > 0)
	{
		...
		TakePictEx.lpstrUNICODE_Cam_Data = _lpszUNICODECamData;
	}
	if (strlen(lpszFilePath) > 0)
	{
		TakePictEx.lpstrPicture_File = (LPTSTR)lpszFilePath;
	}
	else
	{
		TakePictEx.lpstrPicture_File = NULL;
	}

	// ���ֳ����ǰ��ձ�׼XFS320Э����У�û����չwScanSize�ֶΣ��ʲ���ָ��ֵ��ֻ���ڽṹ��ʼ��ʱʹ��Ĭ��ֵ
...
HRESULT CDCCamera_31::CMD_CAM_DISPLAY(long lCamera, long lAction, long lX, long lY, long lWidth, long lHeight)
{
	...
	LFSCAMDISP disp = { 0 };
	disp.wCamera = (WORD)lCamera;
	disp.wAction = (WORD)lAction;
	disp.wX = (WORD)lX;
	disp.wY = (WORD)lY;
	disp.wWidth = (WORD)lWidth;
	disp.wHeight = (WORD)lHeight;
	disp.hWnd = NULL;
	disp.wHpixel = 0;
	disp.wVpixel = 0;
	disp.lpstrText_Data = NULL;
...
CString CDCCamera_31::HandleCmdResult(LPLFSRESULT lpWfsRet)
{
	...
	switch (lpWfsRet->u.dwCommand_Code)
	{
		...
		{
			LPLFSXCAMDATA lpCamData = NULL;
			lpCamData = (LPLFSXCAMDATA)lpWfsRet->lpvdBuffer;

			if (NULL != lpCamData)
			{
				strCmdResult = CString(lpCamData->usLength + 256, 0);
...
CString CDCCamera_31::HandleInfResult(LPLFSRESULT lpWfsRet)
{
	...
	if (LFS_INF_CAM_STATUS == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCAMSTATUS lpStatus = (LPLFSCAMSTATUS)lpWfsRet->lpvdBuffer;
		if (NULL == lpStatus)
		{
			return strWfsResult;
		}
		...
		{
			sprintf((char *)strMedia.c_str(), "%d", lpStatus->wMedia[nLoop]);
		...
		for (int nLoop = 0; nLoop < LFS_CAM_CAMERAS_SIZE; nLoop++)
		{
			sprintf((char *)strCamera.c_str(), "%d", lpStatus->wCameras[nLoop]);
		...
		strWfsResult = CString(strlen(strMedias.c_str()) + strlen(strCameras.c_str()) + strlen(strPictures.c_str()) + (NULL == lpStatus->lpstrExtra ? 0 : strlen(lpStatus->lpstrExtra)) + 2048, 0);
	...
	else if (LFS_INF_CAM_CAPABILITIES == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCAMCAPS lpCaps = (LPLFSCAMCAPS)lpWfsRet->lpvdBuffer;
		if (NULL == lpCaps)
		{
			return strWfsResult;
		}
		...
		for (int nLoop = 0; nLoop < LFS_CAM_CAMERAS_SIZE; nLoop++)
		{
			sprintf((char *)strCamera.c_str(), "%d", lpCaps->wCameras[nLoop]);
		...
		strWfsResult = CString(strlen(strCameras.c_str()) + (NULL == lpCaps->lpstrExtra ? 0 : strlen(lpCaps->lpstrExtra)) + 2048, 0);
		sprintf((char *)strWfsResult.c_str(), "wfsresult.u.dwcommandcode=%d"
												"%s"
												...
												strCameras.c_str(),
												GetStringSplitChar(),
												lpCaps->usMax_Pictures,
												GetStringSplitChar(),
												lpCaps->wCam_Data,
												GetStringSplitChar(),
												lpCaps->usMax_Data_Length,
												GetStringSplitChar(),
