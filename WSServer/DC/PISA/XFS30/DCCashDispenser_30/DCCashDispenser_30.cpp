...
CString CDCCashDispenser_30::HandleEXEEvtResult(LPLFSRESULT lpWfsRet)
{
	...
	switch (lpWfsRet->u.dwEvent_ID)
	{
		...
		case LFS_EXEE_CDM_CASHUNITERROR:
			{
				LPLFSCDMCUERROR lpCashUnitError = (LPLFSCDMCUERROR)lpWfsRet->lpvdBuffer;
				if (NULL == lpCashUnitError)
				{
					break ;
				}

				LPLFSCDMCASHUNIT lpWfsCdmCashUnit = (LPLFSCDMCASHUNIT)lpCashUnitError->lpCash_Unit;
...
CString CDCCashDispenser_30::HandleSRVEvtResult(LPLFSRESULT lpWfsRet)
{
	...
	// 如果需要使用LFS_SRVE_CDM_MEDIADETECTED事件，需要注意WOSA文档中的LFS_SRVE_CDM_MEDIADETECTED事件的参数
	// 定义的是"LPLFSCDMITEMPOSITION * lpItemPosition"
	// 这个有歧义，正常LP??? * 后面的变量名应是lpp开头，如果是lp开头，就应该是只有LP???
	// CIM文档中这个没有问题，没有"*"
	// 另：取款中应作为"LPLFSCDMITEMPOSITION lpItemPosition"（没有"*"），此处不需要指针数组，计算要用指针数组也应该是CIM中先使用
...
CString CDCCashDispenser_30::HandleCmdResult(LPLFSRESULT lpWfsRet)
{
	...
	switch (lpWfsRet->u.dwCommand_Code)
	{
		...
		{
			// 不break ;，下面的LFS_CMD_CDM_DISPENSE分支和LFS_CMD_CDM_DENOMINATE分支都返回LFSCDMDENOMINATION结构
			// break ;
		}
		case LFS_CMD_CDM_DISPENSE:
		{
			// 注意：上面的LFS_CMD_CDM_DENOMINATE分支处理结束后没有break ;，故会重用此分支的代码

			LPLFSCDMDENOMINATION lpDenomination = (LPLFSCDMDENOMINATION)lpWfsRet->lpvdBuffer;
			...
			if (NULL != lpDenomination->lpulValues)
			{
				for (int nLoop = 0; nLoop < lpDenomination->usCount; nLoop ++)
				{
					CString strDenomination(256, 0);
					sprintf((char *)strDenomination.c_str(), "%s"
											"wfsresult.lpbuffer.lpulValues[%d]=%d", 
											GetStringSplitChar(),
											nLoop, lpDenomination->lpulValues[nLoop]);
		...
		{
			LPLFSCDMCOUNT lpCount = (LPLFSCDMCOUNT)lpWfsRet->lpvdBuffer;
			if (NULL == lpCount)
			{
				break ;
			}

			LPLFSCDMCOUNTEDPHYSCU * lppCountedPhysCUs = lpCount->lppCounted_Phys_CUs;
			...
			sprintf((char *)strCmdResult.c_str(), "%s"
								...
								GetStringSplitChar(),
								lpCount->usNum_Physical_CUs);

			for (int nLoop = 0; nLoop < lpCount->usNum_Physical_CUs; nLoop ++)
			{
				if (NULL != lppCountedPhysCUs[nLoop])
				{
					CString strCdmCountedPhysCu((NULL == lppCountedPhysCUs[nLoop]->lpstrPhysical_Position_Name ? 0 : strlen(lppCountedPhysCUs[nLoop]->lpstrPhysical_Position_Name))
												+ sizeof(lppCountedPhysCUs[nLoop]->cUnit_ID)
												+ 1024, 0);
					sprintf((char *)strCdmCountedPhysCu.c_str(), "%s"
												...
												GetStringSplitChar(),
												nLoop, (NULL == lppCountedPhysCUs[nLoop]->lpstrPhysical_Position_Name ? "NULL" : lppCountedPhysCUs[nLoop]->lpstrPhysical_Position_Name),
												GetStringSplitChar(),
												nLoop, CString(lppCountedPhysCUs[nLoop]->cUnit_ID, sizeof(lppCountedPhysCUs[nLoop]->cUnit_ID)).c_str(),
												GetStringSplitChar(),
												nLoop, lppCountedPhysCUs[nLoop]->ulDispensed,
												GetStringSplitChar(),
												nLoop, lppCountedPhysCUs[nLoop]->ulCounted,
												GetStringSplitChar(),
												nLoop, lppCountedPhysCUs[nLoop]->usPhy_Status);
...
int CDCCashDispenser_30::OnDCMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
    {
        ...
        case LFS_GETINFO_COMPLETE:
			...
			if (FALSE == m_bCashUnitInfo4EndExchange)  // LFS_CMD_CDM_END_EXCHANGE后续需要使用此状态信息中的钞箱结构，EndExchange使用完毕后再释放
			{
				LFSFreeResult(lpWfsRet);
			}
			break;
...
CString CDCCashDispenser_30::HandleInfResult(LPLFSRESULT lpWfsRet)
{
	...
	if (LFS_INF_CDM_STATUS == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCDMSTATUS lpStatus = (LPLFSCDMSTATUS)lpWfsRet->lpvdBuffer;
		if (NULL == lpStatus)
		{
			return strWfsResult;
		}

		LPLFSCDMOUTPOS * lppPositions = (LPLFSCDMOUTPOS *)lpStatus->lppPositions;
		...
		if (NULL != lppPositions)
		{
			for (int nLoop = 0; NULL != lppPositions[nLoop]; nLoop ++)
			{
				...
				sprintf((char *)strPosition.c_str(), "wfsresult.lpbuffer.lpppositions[%d].fwposition=%d"
									...
									"%s",
									nLoop, lppPositions[nLoop]->wPosition,
									GetStringSplitChar(),
									nLoop, lppPositions[nLoop]->wShutter,
									GetStringSplitChar(),
									nLoop, lppPositions[nLoop]->wPosition_Status,
									GetStringSplitChar(),
									nLoop, lppPositions[nLoop]->wTransport,
									GetStringSplitChar(),
									nLoop, lppPositions[nLoop]->wTransport_Status,
									GetStringSplitChar());
	...
	else if (LFS_INF_CDM_CAPABILITIES == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCDMCAPS lpCaps = (LPLFSCDMCAPS)lpWfsRet->lpvdBuffer;
		if (NULL == lpCaps)
		{
			return strWfsResult;
		}
		...
		sprintf((char *)strWfsResult.c_str(), "wfsresult.u.dwcommandcode=%d"
			...
			GetStringSplitChar(),
			lpCaps->bIntermediate_Stacker ? "true" : "false",
			GetStringSplitChar(),
	...
	else if (LFS_INF_CDM_CASH_UNIT_INFO == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCDMCUINFO lpCashUnitInfo = (LPLFSCDMCUINFO)lpWfsRet->lpvdBuffer;
		...
		for (int nLoop = 0; nLoop < lpCashUnitInfo->usCount; nLoop++)
		{
			lpList = lpCashUnitInfo->lppList[nLoop];
			if (NULL == lpList)
			{
				continue;
			}

			strCdmCashUnit = CString((NULL == lpList->lpstrCash_Unit_Name ? 0 : strlen(lpList->lpstrCash_Unit_Name))
				+ sizeof(lpList->cUnit_ID)
				+ sizeof(lpList->cCurrency_ID) + 2048, 0);
			sprintf((char *)strCdmCashUnit.c_str(), "%s"
				...
				nLoop, lpList->usType,
				GetStringSplitChar(),
				nLoop, (NULL == lpList->lpstrCash_Unit_Name ? "NULL" : lpList->lpstrCash_Unit_Name),
				GetStringSplitChar(),
				nLoop, CString(lpList->cUnit_ID, sizeof(lpList->cUnit_ID)).c_str(),
				GetStringSplitChar(),
				nLoop, CString(lpList->cCurrency_ID, sizeof(lpList->cCurrency_ID)).c_str(),
				GetStringSplitChar(),
				nLoop, lpList->ulValues,
				GetStringSplitChar(),
				nLoop, lpList->ulInitial_Count,
				GetStringSplitChar(),
				nLoop, lpList->ulCount,
				GetStringSplitChar(),
				nLoop, lpList->ulReject_Count,
				GetStringSplitChar(),
				nLoop, lpList->ulMinimum,
				GetStringSplitChar(),
				nLoop, lpList->ulMaximum,
				GetStringSplitChar(),
				nLoop, (TRUE == lpList->bApp_Lock ? "true" : "false"),
				GetStringSplitChar(),
				nLoop, lpList->usStatus,
				GetStringSplitChar(),
				nLoop, lpList->usNumPhysical_CUs);

			LPLFSCDMPHCU lpPhysical = NULL;
			for (int jLoop = 0; jLoop < lpList->usNumPhysical_CUs; jLoop++)
			{
				lpPhysical = lpList->lppPhysical[jLoop];
				if (NULL == lpPhysical)
				{
					continue;
				}

				strCdmPhCu = CString((NULL == lpPhysical->lpstrPhysical_Position_Name ? 0 : strlen(lpPhysical->lpstrPhysical_Position_Name))
					+ sizeof(lpPhysical->cUnit_ID) + 2048, 0);
				sprintf((char *)strCdmPhCu.c_str(), "%s"
					...
					GetStringSplitChar(),
					nLoop, jLoop, (NULL == lpPhysical->lpstrPhysical_Position_Name ? "NULL" : lpPhysical->lpstrPhysical_Position_Name),
					GetStringSplitChar(),
					nLoop, jLoop, CString(lpPhysical->cUnit_ID, sizeof(lpPhysical->cUnit_ID)).c_str(),
					GetStringSplitChar(),
					nLoop, jLoop, lpPhysical->ulInitial_Count,
					GetStringSplitChar(),
					nLoop, jLoop, lpPhysical->ulCount,
					GetStringSplitChar(),
					nLoop, jLoop, lpPhysical->ulReject_Count,
					GetStringSplitChar(),
					nLoop, jLoop, lpPhysical->ulMaximum,
					GetStringSplitChar(),
					nLoop, jLoop, lpPhysical->usPhy_Status,
					GetStringSplitChar(),
					nLoop, jLoop, (TRUE == lpPhysical->bHardware_Sensor ? "true" : "false"),
					GetStringSplitChar());
		...
		// EXE_LFS_CMD_CDM_END_EXCHANGE方法会调用此GET_LFS_INF_CDM_CASH_UNIT_INFO方法，用于获取传递到SP的加钞的结构
	...
	else if (LFS_INF_CDM_CURRENCY_EXP == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCDMCURRENCYEXP * lppCurrencyExp = (LPLFSCDMCURRENCYEXP *)lpWfsRet->lpvdBuffer;
		...
		if (NULL != lppCurrencyExp)
		{
			...
			{
				strCurrencyExp = CString(sizeof(lppCurrencyExp[nLoop]->cCurrency_ID) + 1024, 0);
	...
	{
		LPLFSCDMMIXTABLE * lppMixTable = (LPLFSCDMMIXTABLE *)lpWfsRet->lpvdBuffer;
		...
		if (NULL != lppMixTable)
		{
			for (int nLoop = 0; NULL != lppMixTable[nLoop]; nLoop++)
			{
				...
				if (NULL != lppMixTable[nLoop]->lpulMix_Header)
				{
					for (int jLoop = 0; jLoop < lppMixTable[nLoop]->usCols; jLoop++)
					{
						CString strMixHeader(256, 0);
						sprintf((char *)strMixHeader.c_str(), "%s"
							"wfsresult.lpbuffer[%d].lpulmixheader[%d]=%d",
							GetStringSplitChar(),
							nLoop, jLoop, lppMixTable[nLoop]->lpulMix_Header[jLoop]);
				...
				LPLFSCDMMIXROW * lppMixRows = (LPLFSCDMMIXROW *)lppMixTable[nLoop]->usRows;
				if (NULL != lppMixRows)
				{
					for (int jLoop = 0; NULL != lppMixRows[jLoop]; jLoop++)
					{
						...
						if (NULL != lppMixRows[jLoop]->lpusMixture)
						{
							for (int kLoop = 0; kLoop < lppMixTable[nLoop]->usCols; kLoop++)
							{
								CString strMixture(256, 0);
								sprintf((char *)strMixture.c_str(), "%s"
									"wfsresult.lpbuffer[%d].lppmixrows[%d].lpusmixture[%d]=%d",
									GetStringSplitChar(),
									nLoop, jLoop, kLoop, lppMixRows[jLoop]->lpusMixture[kLoop]);

								strMixtures = string(strMixtures.c_str()) + strMixture;
							}
						}

						CString strMixRow(strlen(strMixtures.c_str()) + 1024, 0);
						sprintf((char *)strMixRow.c_str(), "wfsresult.lpbuffer[%d].lppmixrows[%d].ulamount=%d"
							"%s"
							"%s",
							nLoop, jLoop, lppMixRows[jLoop]->ulAmount,
							strMixtures.c_str(),
							GetStringSplitChar());
				...
				strMixTable = CString((NULL == lppMixTable[nLoop]->lpstrName ? 0 : strlen(lppMixTable[nLoop]->lpstrName))
					+ strlen(strMixHeaders.c_str()) + strlen(strMixRows.c_str()) + 1024, 0);
				sprintf((char *)strMixTable.c_str(), "wfsresult.lpbuffer[%d].usmixnumber=%d"
					...
					"%s"
					"%s",
					nLoop, lppMixTable[nLoop]->usMix_Number,
					GetStringSplitChar(),
					nLoop, (NULL == lppMixTable[nLoop]->lpstrName ? "NULL" : lppMixTable[nLoop]->lpstrName),
					GetStringSplitChar(),
					nLoop, lppMixTable[nLoop]->usRows,
					GetStringSplitChar(),
					nLoop, lppMixTable[nLoop]->usCols,
					strMixHeaders.c_str(),
					strMixRows.c_str());
	...
	else if (LFS_INF_CDM_PRESENT_STATUS == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSCDMPRESENTSTATUS lpPresentStatus = (LPLFSCDMPRESENTSTATUS)lpWfsRet->lpvdBuffer;
		if (NULL == lpPresentStatus)
		{
			return strWfsResult;
		}

		LPLFSCDMDENOMINATION lpDenomination = lpPresentStatus->lpDenomination;
		...
		if (NULL != lpDenomination)
		{
			...
			strDenomination = CString(strlen(strValues.c_str()) + 1024, 0);
			sprintf((char *)strDenomination.c_str(), "wfsresult.lpbuffer.lpdenomination.ccurrencyid=%s"
				...
				CString(lpDenomination->cCurrency_ID, sizeof(lpDenomination->cCurrency_ID)).c_str(),
				GetStringSplitChar(),
				...
				strValues.c_str(),
				GetStringSplitChar(),
				lpDenomination->ulCash_Box);
		}

		strWfsResult = string(strlen(strDenomination.c_str()) + (NULL == lpPresentStatus->lpstrExtra ? 0 : strlen(lpPresentStatus->lpstrExtra)) + 2048, 0);
		sprintf((char *)strWfsResult.c_str(), "wfsresult.u.dwcommandcode=%d"
			"%s"
			"%s"
			"%s"
			...
			lpPresentStatus->wPresent_State,
			GetStringSplitChar(),
			(NULL == lpPresentStatus->lpstrExtra ? "NULL" : FormatExtra(lpPresentStatus->lpstrExtra)));
	}
...
HRESULT CDCCashDispenser_30::CMD_CDM_COUNT(LPCTSTR strPhysicalLocation, long lPosition) 
{
	...
	if (0 == stricmp(strPhysicalLocation, "_ALL_"))  // 自定义关键字，传入 _ALL_ 时将钞箱中所有钞票清空并计数
	{
		PhysicalCU.bEmpty_All = TRUE;
	}
	else
	{
		PhysicalCU.bEmpty_All = FALSE;
	}
	PhysicalCU.wPosition = (WORD)lPosition;
	PhysicalCU.lpstrPhysical_Position_Name = (LPTSTR)strPhysicalLocation;
...
HRESULT CDCCashDispenser_30::CMD_CDM_DENOMINATE(long lAmount, LPCTSTR strCurrency, long lMixAlgorithm) 
{
	...
	Denominate.usMix_Number = (USHORT)lMixAlgorithm;
	LFSCDMDENOMINATION Denomination = {0};
	if (strlen(strCurrency) > sizeof(Denomination.cCurrency_ID))
	{
		memcpy(Denomination.cCurrency_ID, strCurrency, sizeof(Denomination.cCurrency_ID));
	}
	else
	{
		memcpy(Denomination.cCurrency_ID, strCurrency, strlen(strCurrency));
	}
	Denomination.ulAmount = lAmount;
	Denomination.usCount = 0;
	Denomination.ulCash_Box = 0;
...
HRESULT CDCCashDispenser_30::CMD_CDM_DISPENSE(long lAmount, LPCTSTR strCurrency, long lMixAlgorithm, long lPosition, BOOL bPresent)
{
	...
	Dispense.usMix_Number = (USHORT)lMixAlgorithm;
	Dispense.wPosition = (WORD)lPosition;
	Dispense.bPresent = bPresent;
...
HRESULT CDCCashDispenser_30::CMD_CDM_END_EXCHANGE(LPCTSTR strCUInfo) 
{
	...
	// 调用GET_LFS_INF_CDM_CASH_UNIT_INFO获取钞箱结构
	...
	LPLFSCDMCUINFO lpCashUnitInfo = (LPLFSCDMCUINFO)m_lpResult4EndExchange->lpvdBuffer;
	if (NULL == lpCashUnitInfo || NULL == lpCashUnitInfo->lppList)
	{
		m_bCashUnitInfo4EndExchange = FALSE;
	...
	{
		// 不改变钞箱张数的加钞，用于取款模块复位、降钞箱等操作中的更换钞箱命令，或者设置钞箱币种面值的操作
 	...
 	for (int nLoop = 0; nLoop < lpCashUnitInfo->usCount; nLoop ++)
	{
		...
		if (NULL != lpList)
		{
			if (FALSE == bEndWithNoChange)
			{
				...
				lpList->ulInitial_Count = lpList->ulCount;
				lpList->ulReject_Count = 0;
			}
			else
			{
				// 钞箱计数不改变
				...
				if (TRUE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
				{
					if (strlen(strParamValue.c_str()) > 3)
					{
						...
						printf("if (strlen(strParamValue.c_str()) > 3)\n");
						return (short)DC_ERROR;
					}

					memset(lpList->cCurrency_ID, 0, sizeof(lpList->cCurrency_ID));
					memcpy(	lpList->cCurrency_ID, 
							strParamValue.c_str(),
				...
				if (TRUE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
				{
					lpList->ulValues = atoi((LPCTSTR)strParamValue.c_str());
				}
			}
			...
			for (int jLoop = 0; jLoop < lpList->usNumPhysical_CUs; jLoop ++)
			{
				...
				if (NULL != lpPhysical)
				{
					if (FALSE == bEndWithNoChange)
					{
						...
						lpPhysical->ulCount = atoi(strParamValue.c_str());
						lpPhysical->ulInitial_Count = lpPhysical->ulCount;
						lpPhysical->ulReject_Count = 0;
					}
					else
					{
...
HRESULT CDCCashDispenser_30::CMD_CDM_RESET(long lNumber, long lRetractArea, long lRetractIndex, long lPosition) 
{
	...
	if (lNumber >= 0)
	{
		Retract.wOutput_Position = (WORD)lPosition;
		Retract.usRetract_Area = (USHORT)lRetractArea;
		Retract.usIndex = (USHORT)lRetractIndex;

		ResetIn.wOutput_Position = Retract.wOutput_Position;
...
HRESULT CDCCashDispenser_30::CMD_CDM_TEST_CASH_UNITS(long lNumber, long lRetractArea, long lRetractIndex, long lPosition) 
{
	...
	Position.wOutput_Position = Retract.wOutput_Position;
	Position.usNumber = (USHORT)lNumber;
	Position.lpRetract_Area = &Retract;
