...
CString CDCSensorsAndIndicators_30::HandleSRVEvtResult(LPLFSRESULT lpWfsRet)
{
	...
	switch (lpWfsRet->u.dwEvent_ID)
	{
		...
		{
			CString strResult;
			LPLFSSIUPORTEVENT lpPortEvent = (LPLFSSIUPORTEVENT)lpWfsRet->lpvdBuffer;
			if (NULL == lpPortEvent)
			{
				break ;
			}

			strEvtResult = CString((NULL == lpPortEvent->lpstrExtra ? 0 : strlen(lpPortEvent->lpstrExtra)) + 1024, 0);
			sprintf((char *)strEvtResult.c_str(), "%s"
								...
								GetStringSplitChar(),
								lpPortEvent->wPort_Type,
								GetStringSplitChar(),
								lpPortEvent->wPort_Index,
								GetStringSplitChar(),
								lpPortEvent->wPort_Status,
								GetStringSplitChar(),
								(NULL == lpPortEvent->lpstrExtra ? "NULL" : FormatExtra(lpPortEvent->lpstrExtra)));
...
CString CDCSensorsAndIndicators_30::HandleInfResult(LPLFSRESULT lpWfsRet)
{
	...
	if (LFS_INF_SIU_STATUS == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSSIUSTATUS lpStatus = (LPLFSSIUSTATUS)lpWfsRet->lpvdBuffer;
		if (NULL == lpStatus)
		{
			return strWfsResult;
		}
		...
		int nLoop = 0;
		CString strSensors = "";
		for (nLoop = 0; nLoop <= LFS_SIU_ENHANCEDAUDIO; nLoop ++)  // ����Ҫȡ��������Ĵ�СLFS_SIU_SENSORS_SIZE(32)
		{
			CString strSensor(256, 0);
			sprintf((char *)strSensor.c_str(), "%s"
							"wfsresult.lpbuffer.fwsensors[%d]=%d", 
							GetStringSplitChar(),
							nLoop, lpStatus->wSensors[nLoop]);
	...
	else if (LFS_INF_SIU_CAPABILITIES == lpWfsRet->u.dwCommand_Code)
	{
		LPLFSSIUCAPS lpCaps = (LPLFSSIUCAPS)lpWfsRet->lpvdBuffer;
		if (NULL == lpCaps)
		{
			return strWfsResult;
		}
		...
		for (nLoop = 0; nLoop <= LFS_SIU_HEATING; nLoop++)  // ����Ҫȡ��������Ĵ�СLFS_SIU_INDICATORS_SIZE(16)
		{
			...
			sprintf((char *)strIndicator.c_str(), "%s"
				"wfsresult.lpbuffer.fwindicators[%d]=%d",
				GetStringSplitChar(),
				nLoop, lpCaps->wIndicators[nLoop]);
		...
		for (nLoop = 0; nLoop <= LFS_SIU_HEATING; nLoop++)  // ����Ҫȡ��������Ĵ�СLFS_SIU_AUXILIARIES_SIZE(16)
		{
			...
			sprintf((char *)strAuxiliarie.c_str(), "%s"
				"wfsresult.lpbuffer.fwauxiliaries[%d]=%d",
				GetStringSplitChar(),
				nLoop, lpCaps->wAuxiliaries[nLoop]);
		...
		for (nLoop = 0; nLoop <= LFS_SIU_SCANNER; nLoop++)  // ����Ҫȡ��������Ĵ�СLFS_SIU_GUIDLIGHTS_SIZE(16)
		{
			...
			sprintf((char *)strGuidLight.c_str(), "%s"
				"wfsresult.lpbuffer.fwguidlights[%d]=%d",
				GetStringSplitChar(),
				nLoop, lpCaps->wGuid_Lights[nLoop]);

			strGuidLights = string(strGuidLights.c_str()) + strGuidLight;
		}

		strWfsResult = CString(strlen(strSensors.c_str()) + strlen(strDoors.c_str()) + strlen(strSensorsAndIndicators.c_str()) + strlen(strAuxiliaries.c_str()) + strlen(strGuidLights.c_str())
			+ (NULL == lpCaps->lpstrExtra ? 0 : strlen(lpCaps->lpstrExtra)) + 1024, 0);
		sprintf((char *)strWfsResult.c_str(), "wfsresult.u.dwcommandcode=%d"
			"%s"
...
HRESULT CDCSensorsAndIndicators_30::CMD_SIU_ENABLE_EVENTS(LPCTSTR strEnable) 
{
	...
	for (int nLoop = 0; nLoop < LFS_SIU_SENSORS_SIZE; nLoop ++)
	{
		...
		{
			Enable.wSensors[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			Enable.wSensors[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_DOORS_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			Enable.wDoors[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			Enable.wDoors[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_INDICATORS_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			Enable.wIndicators[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			Enable.wIndicators[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_AUXILIARIES_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			Enable.wAuxiliaries[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			Enable.wAuxiliaries[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_GUIDLIGHTS_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			Enable.wGuid_Lights[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			Enable.wGuid_Lights[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	{
		Enable.lpstrExtra = (LPTSTR)(LPCTSTR)strParamValue.c_str();
	}
...
HRESULT CDCSensorsAndIndicators_30::CMD_SIU_SET_AUXILIARY(long lAuxiliary, long lCommand) 
{
	...
	SetAuxiliary.wAuxiliary = (WORD)lAuxiliary;
	SetAuxiliary.wCommand = (WORD)lCommand;
...
HRESULT CDCSensorsAndIndicators_30::CMD_SIU_SET_DOOR(long lDoor, long lCommand) 
{
	...
	SetDoor.wDoor = (WORD)lDoor;
	SetDoor.wCommand = (WORD)lCommand;
...
HRESULT CDCSensorsAndIndicators_30::CMD_SIU_SET_GUIDLIGHT(long lGuidLight, long lCommand) 
{
	...
	SetGuidLight.wCommand = (WORD)lCommand;
...
HRESULT CDCSensorsAndIndicators_30::CMD_SIU_SET_INDICATOR(long lIndicator, long lCommand) 
{
	...
	SetIndicator.wIndicator = (WORD)lIndicator;
	SetIndicator.wCommand = (WORD)lCommand;
...
HRESULT CDCSensorsAndIndicators_30::CMD_SIU_SET_PORTS(LPCTSTR strSetPorts) 
{
	...
	for (int nLoop = 0; nLoop < LFS_SIU_DOORS_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			SetPorts.wDoors[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			SetPorts.wDoors[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_INDICATORS_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			SetPorts.wIndicators[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			SetPorts.wIndicators[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_AUXILIARIES_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			SetPorts.wAuxiliaries[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			SetPorts.wAuxiliaries[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	for (int nLoop = 0; nLoop < LFS_SIU_GUIDLIGHTS_SIZE; nLoop ++)
	{
		...
		if (FALSE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
		{
			SetPorts.wGuid_Lights[nLoop] = LFS_SIU_NO_CHANGE;
		}
		else
		{
			SetPorts.wGuid_Lights[nLoop] = atoi(strParamValue.c_str());
		}
	}
	...
	if (TRUE == CJSStringParam::GetStringParam(strarrParams, strParamKey.c_str(), strParamValue))
	{
		SetPorts.lpstrExtra = (char *)strParamValue.c_str();
	}
